...
import { listRoadmap, saveRoadmap } from '@/src/backend/modules/presentation/planning/handlers';
...
export async function GET(_: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const result = await listRoadmap(id);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const result = await saveRoadmap(id, items);
  return NextResponse.json(result);
}
