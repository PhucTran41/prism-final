...
import { listEpics, saveEpics } from '@/src/backend/modules/presentation/planning/handlers';
...
export async function GET(_: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const result = await listEpics(id);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const items = Array.isArray(body?.epics) ? body.epics : [];
  const result = await saveEpics(id, items);
  return NextResponse.json(result);
}
